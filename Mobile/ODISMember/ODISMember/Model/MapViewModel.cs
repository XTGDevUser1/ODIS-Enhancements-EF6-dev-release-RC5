...
namespace ODISMember.Model
{
    public class MapViewModel : INotifyPropertyChanged
    {
        ...
        public TKCustomMapPin _selectedPin;
        ...
        public Command<Position> MapLongPressCommand
        {
            get
            {
                ...
                {
                    string title = title = string.Format("Pin {0}, {1}", position.Latitude, position.Longitude);
                    ...
                    ODISMember.Entities.Model.AddressBing address = null;
                        ...
                        if (bingAddressRoot.ResourceSets != null && bingAddressRoot.ResourceSets.Count > 0)
                        {
                            if (bingAddressRoot.ResourceSets[0].Resources != null && bingAddressRoot.ResourceSets[0].Resources.Count > 0)
                            {
                                if (bingAddressRoot.ResourceSets[0].Resources[0].Address != null)
                                {
                                    ...
                                    title = bingAddressRoot.ResourceSets[0].Resources[0].Address.FormattedAddress;
                                }
                            }
                        }
                    }
                    ...
                    if (address != null)
                    {
                        ...
                        {
                            pin.LocationAddress = address.FormattedAddress;
                        }
                        else
                        {
        ...
        public Command<IPlaceResult> PlaceSelectedCommand
        {
            get
            {
                return new Command<IPlaceResult>(async p =>
                {
                    ...
                    TK.CustomMap.Api.Bing.ResourceBing bingAddressRoot = null;
                    TK.CustomMap.Api.Bing.AddressBing address = null;
                    ...
                    bingAddressRoot = p as TK.CustomMap.Api.Bing.ResourceBing;
                    address = bingAddressRoot.Address;
                    if (bingAddressRoot != null)
                    {

                        if (bingAddressRoot != null)
                        {
                            if (bingAddressRoot.Point != null)
                            {
                                this.MapCenter = new Position(bingAddressRoot.Point.Coordinates[0], bingAddressRoot.Point.Coordinates[1]);
                                //return;
                            }
                        }
                    ...
                    if (Device.OS == TargetPlatform.Android && bingAddressRoot == null)
                    {
                    ...
                    else if (Device.OS == TargetPlatform.iOS && bingAddressRoot == null)
                    {
                        var prediction = (TKNativeiOSPlaceResult)p;

                        this.MapCenter = prediction.Details.Coordinate;
                    }
                    /*BEGIN: Custom Written*/
                    if (this.MapCenter != null)
                    {
                        var pin = new TKCustomMapPin
                        {
                            Position = new Position(this.MapCenter.Latitude, this.MapCenter.Longitude),
                            Title = p.Description,
                            ShowCallout = true,
                            IsDraggable = true,
                            DefaultPinColor = Color.Red
                        ...
                        //Device.BeginInvokeOnMainThread(() =>
                        //{
                        //    this.MapRegion = MapSpan.FromCenterAndRadius(pin.Position, Distance.FromMiles(2));// this.MapRegion.Radius);
                        //    this._pins.Clear();
                        //    this._pins.Add(pin);
                        //    this.SelectedPin = pin;
                        //});
        ...
        public Command<TKCustomMapPin> DragEndCommand
        {
            get
            {
                return new Command<TKCustomMapPin>(async pin =>
                {
                    ...
                    TKCustomMapPin tkCustomMapPin = pin as TK.CustomMap.TKCustomMapPin;
                    string title = string.Empty;
                    MemberHelper memberHelper = new MemberHelper();
                    BingAddressRoot bingAddressRoot = await memberHelper.GetBingAddress(tkCustomMapPin.Position.Latitude.ToString(), tkCustomMapPin.Position.Longitude.ToString());
                    ODISMember.Entities.Model.AddressBing address = null;
                    if (bingAddressRoot != null)
                    {
                        if (bingAddressRoot.ResourceSets != null && bingAddressRoot.ResourceSets.Count > 0)
                    ...
                    tkCustomMapPin.Title = title;


                    if (address != null)
                    {
                        tkCustomMapPin.LocationCity = address.Locality;
                        tkCustomMapPin.LocationStateProvince = address.AdminDistrict;
                        tkCustomMapPin.LocationPostalCode = address.PostalCode;
                        if (address.AddressLine == null)
                        {
                            tkCustomMapPin.LocationAddress = address.FormattedAddress;
                        }
                        else
                        {
                            tkCustomMapPin.LocationAddress = address.AddressLine;
                        }
                        tkCustomMapPin.LocationCountryCode = address.CountryRegionIso2;
                        tkCustomMapPin.LocationDescription = address.FormattedAddress;
                    }
                    Device.BeginInvokeOnMainThread(() =>
                    {
                        this._pins.Clear();
                        this._pins.Add(tkCustomMapPin);
                        this.SelectedPin = tkCustomMapPin;
                    });
                });
            }
        }
        ...
        public MapViewModel(double latitude = 39.023407, double longitude = -104.784104)
        {
            //Default to United States lat lng                 
            this._mapCenter = new Position(latitude, longitude);
