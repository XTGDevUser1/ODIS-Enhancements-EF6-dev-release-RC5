...
namespace ODISMember.CustomControls
{
	...
	{
		View _currentView;
		StackLayout dotLayout,zoomLayout;
		ExtendedFrame exFrame;
		double _height, _width;
		int dotCount = 1;
		Button mRedDot;
		Action zoomButtonClicked;
		Rectangle framePosition,zoomPosition;
		public SliderView (View rootview, double height, double width, Action zoomButtonClicked = null,string frameText = null) 
		{
			//Set the height and width to be used throughout the SLiderView
			_height = height;
			_width = width;
			this.zoomButtonClicked = zoomButtonClicked;
			...
			if (zoomButtonClicked == null) {
				ViewScreen.VerticalOptions = LayoutOptions.Start;
			}
			...
			Image zoomImage = new Image{
				Source = "zoom_in.png"
			};
			var sliderTapGestureRecognizer = new TapGestureRecognizer() { NumberOfTapsRequired = 1 };
			sliderTapGestureRecognizer.Tapped += onSliderZoomClicked;
			zoomImage.GestureRecognizers.Add (sliderTapGestureRecognizer);
			zoomLayout.Children.Add (zoomImage);
			mRedDot.Clicked += onSliderZoomClicked;
			if (zoomButtonClicked == null) {
				zoomLayout.IsVisible = false;
			}
		//	zoomLayout.Children.Add (mRedDot);

			zoomPosition = new Rectangle (
				//.5,1,.5,.1
				x: _width-40, //_width-65,
				y:_height - 40,
				width: 30,
				height: 30
			);
			...
			//Layout the zoom button to Viewscreen --@Venkatesh
			ViewScreen.Children.Add (zoomLayout,zoomPosition);

			if (!string.IsNullOrEmpty(frameText)) {
				exFrame = new ExtendedFrame ("distance_pointer.png",frameText);
				ViewScreen.Children.Add (exFrame, framePosition);
			}
		...
		//@Venkatesh For zoom slider images
		void onSliderZoomClicked(object sender, EventArgs e)
		{
		...
		public ExtendedFrame DistanceFrame{
			get{ 
				return exFrame;
			}
		}
		public Rectangle FrameBoxView{
			get{ 
				return framePosition;
			}
		}

		public StackLayout ZoomLayout{
			get{ 
				return zoomLayout;
			}
		}
		public Rectangle ZoomPosition{
			get{ 
				return zoomPosition;
			}
		}
