...
namespace ODISMember.CustomControls
{
    ...
    public class BindableRadioGroup : StackLayout
    {
        ...
        private static void OnItemsSourceChanged(BindableObject bindable, IEnumerable oldValue, IEnumerable newValue)
        {
            ...
            radButtons.VerticalOptions = LayoutOptions.CenterAndExpand;
            radButtons.HorizontalOptions = LayoutOptions.FillAndExpand;
            ...
            {
                RadioButtonItem radioItem = (RadioButtonItem)item;
                var button = new CustomRadioButton
                {
                    Text = radioItem.Text.ToString(),//item.ToString(),
                    Id = radIndex++,
                    TextColor = radButtons.TextColor,
                    FontSize = Device.GetNamedSize(NamedSize.Small, radButtons),
                    FontName =FontResources.BaseLightFontName //radButtons.FontName
                };
                Image img = new Image()
                {
                    HeightRequest = 30,
                    WidthRequest = 30,
                    Source = radioItem.SelectedImageURL,
                ...
                grid.Children.Add(img, 1, 2, 0, 1);



                radButtons.Children.Add(grid);
