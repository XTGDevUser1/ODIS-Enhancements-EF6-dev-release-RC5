...
namespace ODISMember.Droid.ResizeImage
{
    [Activity]
    public class CorpImage : MonitoredActivity
    {
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            FindViewById<Button>(Resource.Id.discard).Click += (sender, e) => { SetResult(Result.Canceled); Finish(); RootPage.PopModal(); };
        ...
        private void onSaveClicked()
        {
                ...
                (myExtras.GetParcelable("data") != null || myExtras.GetBoolean("return-data")))
            {
                ...
                using (var stream = new MemoryStream())
                {
                    ...
                    Global.CroppedImage = bitmapData;
                }

                //Bitmap originalImage = BitmapFactory.DecodeByteArray(bitmapData, 0, bitmapData.Length);
                //Bitmap CompImage = getResizedBitmap(originalImage, 200);

                //var sdCardPath = Android.OS.Environment.ExternalStorageDirectory.AbsolutePath;
                //var filePath = System.IO.Path.Combine(sdCardPath, "NintyPic.png");
                //var stream1 = new FileStream(filePath, FileMode.Create);
                ////originalImage.Compress(Bitmap.CompressFormat.Png, 0, stream1);
                //CompImage.Compress(Bitmap.CompressFormat.Png,90, stream1);
                //stream1.Close();

                Finish();
                //removing cropview page from the navigation stack
                RootPage.PopModal();
            }
            else
            {
        ...
        private void saveOutput(Bitmap croppedImage)
        {
            if (saveUri != null)
            {
                ...
                //Xamarin.Forms.MessagingCenter.Send<MessageSender, string>(new MessageSender(), "URL", saveUri.Path.ToString());
                Bundle extras = new Bundle();
