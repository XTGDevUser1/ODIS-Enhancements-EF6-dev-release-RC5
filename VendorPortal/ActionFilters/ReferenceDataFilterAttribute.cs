    ...
    public class ReferenceDataFilterAttribute : ActionFilterAttribute
    {
        ...
        public override void OnResultExecuting(ResultExecutingContext filterContext)
        {
            ...
            switch (_key)
            {
                ...
                case StaticData.ReferralSource:
                    viewData[StaticData.ReferralSource.ToString()] = ReferenceDataRepository.GetReferralSources().ToSelectListItem<VendorApplicationReferralSource>(x => x.ID.ToString(), y => y.Description, addDefaultValue);
                    break;
                ...
                case StaticData.ACHAccountType:
                    viewData[StaticData.ACHAccountType.ToString()] = ReferenceDataRepository.GetACHAccountTypes().ToSelectListItem<DropDownEntityForString>(x => x.Text.ToString(), y => y.Value, addDefaultValue);
                    break;
                case StaticData.VendorInfoTaxClassification:
                    viewData[StaticData.VendorInfoTaxClassification.ToString()] = ReferenceDataRepository.GetVendorInfoTaxClassifications().ToSelectListItem<DropDownEntityForString>(x => x.Text.ToString(), y => y.Value, addDefaultValue);
                    break;
                case StaticData.VendorPortalContactUsSubject:
                    viewData[StaticData.VendorPortalContactUsSubject.ToString()] = ReferenceDataRepository.VendorPortalContactUsSubject().ToSelectListItem<DropDownEntityForString>(x => x.Text.ToString(), y => y.Value, addDefaultValue);
                    break;
                case StaticData.UserRoles:
                    viewData[StaticData.UserRoles.ToString()] = ReferenceDataRepository.GetRoles("VendorPortal").Where(a => a.RoleName != "sysadmin").ToSelectListItem<aspnet_Roles>(x => x.RoleName.ToString(), y => y.RoleName, addDefaultValue);
                    break;
                case StaticData.DispatchSoftwareProduct:
                    viewData[StaticData.DispatchSoftwareProduct.ToString()] = ReferenceDataRepository.GetDispatchSoftwareProduct(false).OrderBy(a => a.Sequence).ToSelectListItem<DispatchSoftwareProduct>(x => x.ID.ToString(), y => y.SoftwareName, addDefaultValue);
                    break;
                case StaticData.DispatchGPSNetwork:
                    viewData[StaticData.DispatchGPSNetwork.ToString()] = ReferenceDataRepository.GetDispatchGPSNetwork().OrderBy(a => a.Sequence).ToSelectListItem<DispatchGPSNetwork>(x => x.ID.ToString(), y => y.Description, addDefaultValue);
                    break;
            }
