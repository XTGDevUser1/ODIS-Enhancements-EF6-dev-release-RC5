...
INSERT INTO [Event]
     ...
     VALUES
           (
			...
			, (SELECT ID FROM EventCategory WHERE Name = 'Insurance') 
			, 'InsuranceExpiring', 'Insurance expiring', 1, 1, NULL, NULL
			)		

END

IF NOT EXISTS(SELECT * FROM [Event] where Name='InsuranceExpired')
BEGIN
INSERT INTO [Event]
           ([EventTypeID]
           ,[EventCategoryID]
     ...
     VALUES
           (
			(SELECT ID FROM EventType WHERE Name = 'System') 
			, (SELECT ID FROM EventCategory WHERE Name = 'Insurance') 
			, 'InsuranceExpired', 'Insurance certificate has expired', 1, 1, NULL, NULL
			)
END

IF NOT EXISTS(SELECT * FROM [Event] where Name='InsurancePrompt')
BEGIN
INSERT INTO [Event]
           ([EventTypeID]
           ,[EventCategoryID]
     ...
     VALUES
           (
			(SELECT ID FROM EventType WHERE Name = 'System') 
			, (SELECT ID FROM EventCategory WHERE Name = 'Insurance') 
			, 'InsurancePrompt', 'Insurance Prompt', 1, 1, NULL, NULL
			)
END

IF NOT EXISTS(SELECT * FROM PostLoginPrompt where Name='InsuranceExpiring')
BEGIN
INSERT INTO PostLoginPrompt VALUES('InsuranceExpiring','Insurance Expiring','/ISP/Dashboard/GetInsurancePrompt',1,'system',GETDATE())
END
GO


DECLARE @insuranceExpiringEventID INT= NULL,
		@insuranceExpiredEventID INT = NULL,
		@vendorInsuranceTemplateID INT = NULL		


SELECT @insuranceExpiringEventID = ID FROM Event WHERE Name = 'InsuranceExpiring'
SELECT @insuranceExpiredEventID = ID FROM Event WHERE Name = 'InsuranceExpired'

SELECT @vendorInsuranceTemplateID = ID FROM Template WHERE Name = 'Vendor_InsuranceExpiration'


IF NOT EXISTS (SELECT * FROM EventTemplate WHERE EventID = @insuranceExpiringEventID AND TemplateID = @vendorInsuranceTemplateID)
BEGIN
	
	INSERT INTO EventTemplate (EventID, TemplateID,IsDefault)
	SELECT @insuranceExpiringEventID, @vendorInsuranceTemplateID,0
	PRINT ' Inserted EventTemplate for InsuranceExpiring Event'
END

IF NOT EXISTS (SELECT * FROM EventTemplate WHERE EventID = @insuranceExpiredEventID AND TemplateID = @vendorInsuranceTemplateID)
BEGIN
	
	INSERT INTO EventTemplate (EventID, TemplateID,IsDefault)
	SELECT @insuranceExpiredEventID, @vendorInsuranceTemplateID,0

	PRINT ' Inserted EventTemplate for InsuranceExpired Event'
END

IF NOT EXISTS (SELECT * FROM EventSubscription WHERE EventID = (SELECT ID FROM Event WHERE Name = 'InsuranceExpiring'))
BEGIN
	INSERT INTO EventSubscription (EventID, ContactMethodID,EventTemplateID,IsActive,CreateDate, CreateBy, NotificationRecipientTypeID)
	SELECT	(SELECT ID FROM Event WHERE Name = 'InsuranceExpiring'),
			(SELECT ID FROM ContactMethod WHERE Name = 'Email'),
			(SELECT ID FROM EventTemplate WHERE EventID = @insuranceExpiringEventID AND TemplateID = @vendorInsuranceTemplateID),
			1,
			GETDATE(),
			'system',
			(SELECT ID FROM NotificationRecipientType WHERE Name = 'CurrentUser')

	PRINT ' Inserted EventSubscription for InsuranceExpiring Event'

END

IF NOT EXISTS (SELECT * FROM EventSubscription WHERE EventID = (SELECT ID FROM Event WHERE Name = 'InsuranceExpired'))
BEGIN
	INSERT INTO EventSubscription (EventID, ContactMethodID,EventTemplateID,IsActive,CreateDate, CreateBy, NotificationRecipientTypeID)
	SELECT	(SELECT ID FROM Event WHERE Name = 'InsuranceExpired'),
			(SELECT ID FROM ContactMethod WHERE Name = 'Email'),
			(SELECT ID FROM EventTemplate WHERE EventID = @insuranceExpiredEventID AND TemplateID = @vendorInsuranceTemplateID),
			1,
			GETDATE(),
			'system',
			(SELECT ID FROM NotificationRecipientType WHERE Name = 'CurrentUser')

	PRINT ' Inserted EventSubscription for InsuranceExpired Event'
END
