IF EXISTS (SELECT * FROM dbo.sysobjects 
 WHERE id = object_id(N'[dbo].[dms_CustomerFeedback_Activity_List_Get]')   		AND type in (N'P', N'PC')) 
 BEGIN
 DROP PROCEDURE [dbo].[dms_CustomerFeedback_Activity_List_Get] 
 END 
 GO  
 SET ANSI_NULLS ON 
 GO 
 ...
 GO 
  ...
  --EXEC [dms_CustomerFeedback_Activity_List_Get] @CustomerFeedbackID = 96
 CREATE PROCEDURE [dbo].[dms_CustomerFeedback_Activity_List_Get](  
  @CustomerFeedbackID INT = NULL -- TODO - Let's use this in the where clause.   
...
DECLARE @CustomerFeedbackEntityID INT  
DECLARE @ContactLogEntityID INT  
  

SELECT @CustomerFeedbackEntityID =ID from dbo.Entity(NOLOCK) WHERE Name = 'CustomerFeedback'  
...
WHERE   
 ...
 (CLL.EntityID = @CustomerFeedbackEntityID AND CLL.RecordID = @CustomerFeedbackID)  
...
(ELL.EntityID = @CustomerFeedbackEntityID AND ELL.RecordID = @CustomerFeedbackID)  
...
UNION ALL  
 ...
 (C.EntityID = @CustomerFeedbackEntityID AND C.RecordID = @CustomerFeedbackID)  
 
 ORDER BY CreateDate DESC  
  
  
    ;WITH wELUpdateCustomerFeedback
  AS
 (
	SELECT  ID,
			Type,
	...
	WHERE	ISNULL(Name,'') = 'UpdateCustomerFeedback'
 )
  UPDATE @tmpFinalResults
 SET	Comments = CASE WHEN ISNULL(T.[Description],'') = '' THEN '' 
						ELSE [dbo].[fn_EventDetailForFinish_Get](T.[Data],'UpdateCustomerFeedback') END,
		[Description] = 'Update Customer Feedback',
		[Data]=''
 FROM	wELUpdateCustomerFeedback W 
 JOIN	@tmpFinalResults T ON W.ID = T.ID AND W.Type = T.Type

     ;WITH wELCloseCustomerFeedback
  AS
 (
	SELECT  ID,
			Type,
			...
			Comments
	FROM	@tmpFinalResults
	WHERE	ISNULL(Name,'') = 'CloseCustomerFeedback'
 )
  UPDATE @tmpFinalResults
 SET	Comments = CASE WHEN ISNULL(T.[Description],'') = '' THEN '' 
						ELSE [dbo].[fn_EventDetailForFinish_Get](T.[Data],'CloseCustomerFeedback') END,
		[Description] = 'Close Customer Feedback',
		[Data]=''
 FROM	wELCloseCustomerFeedback W 
 JOIN	@tmpFinalResults T ON W.ID = T.ID AND W.Type = T.Type
