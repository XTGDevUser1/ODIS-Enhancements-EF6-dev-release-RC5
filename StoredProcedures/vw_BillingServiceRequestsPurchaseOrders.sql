...
select ServiceRequestID,  
  ...
  IsImpoundRelease,  
  IsOutOfWarranty,  
  IsTransferredCallToAgero,
  IsTransferredCallToAAA,
  ...
  AccountingInvoiceBatchID_VendorInvoice,
  VendorInvoicePaymentDate,
  VendorInvoicePaymentAmount,
...
--- ^ 4 Updated to use ClientReference for PDG     
    ...
    ca.VehicleMake,  
    ca.VehicleModel,  
    ISNULL(dbo.fnc_BillingVINModelYear(ca.VehicleVIN),'Unidentified') as VINModelYear,  
    ...
    convert(date, po.IssueDate) as PurchaseOrderDate,  
    ...
    cast(  
     ...
     when -- 3. When Member Data, Vehicle Data, Is of Dispatch Concern, and Location then DispatchIntended  
       (mbr.ID is not null -- Member Data  
     ...
     cast(  
     case  
      when isnull(CLOG.TransferredCallToAgeroContact, 0) > 0 then 1  
      else 0  
     end as int) as IsTransferredCallToAgero,  
     cast(  
     case  
      when isnull(CLOG.TransferredCallToAAAContact, 0) > 0 then 1  
      else 0  
     end as int) as IsTransferredCallToAAA,  
     ...
     vi.AccountingInvoiceBatchID as AccountingInvoiceBatchID_VendorInvoice,
     vi.PaymentDate as VendorInvoicePaymentDate,
     vi.PaymentAmount as VendorInvoicePaymentAmount,
  ...
  left outer join dbo.VehicleCategory vc with (nolock) on vc.ID = COALESCE(popr.VehicleCategoryID, ca.VehicleCategoryID, 1)  
  ...
  left outer join -- Contact Logs  
    ...
    (select sr2.ID as ServiceRequestID,  
      -- Inbound  
      sum(case when cl.Direction = 'Inbound' then 1 else 0 end) as InboundContactsTOTAL,  
      sum(case when cl.Direction = 'Inbound' and cc.Name = 'NewCall' then 1 else 0 end) as InboundContactsNEWCALL,  
      sum(case when cl.Direction = 'Inbound' and cc.Name in ('ContactCustomer', 'CustomerCallback') then 1 else 0 end) as InboundContactsCUSTOMER,  
      sum(case when cl.Direction = 'Inbound' and cc.Name in ('ContactVendor', 'VendorCallback', 'VendorSelection') then 1  else 0 end) as InboundContactsVENDOR,  
      sum(case when cl.Direction = 'Inbound' and cc.Name in ('ClosedLoop') then 1 else 0 end) as InboundContactsCLOSEDLOOP,  
      sum(case when cl.Direction = 'Inbound' and cc.Name not in   
         ('NewCall', 'ContactCustomer', 'CustomerCallback', 'ContactVendor', 'VendorCallback', 'VendorSelection', 'ClosedLoop')  
         then 1 else 0 end) as InboundContactsOTHER,  
      -- Outbound  
      sum(case when cl.Direction = 'Outbound' then 1 else 0 end) as OutboundContactsTOTAL,  
      sum(case when cl.Direction = 'Outbound' and cc.Name = 'NewCall' then 1 else 0 end) as OutboundContactsNEWCALL,  
      sum(case when cl.Direction = 'Outbound' and cc.Name in ('ContactCustomer', 'CustomerCallback') then 1 else 0 end) as OutboundContactsCUSTOMER,  
      sum(case when cl.Direction = 'Outbound' and cc.Name in ('ContactVendor', 'VendorCallback', 'VendorSelection') then 1 else 0 end) as OutboundContactsVENDOR,  
      sum(case when cl.Direction = 'Outbound' and cc.Name in ('ClosedLoop') then 1 else 0 end) as OutboundContactsCLOSEDLOOP,  
      sum(case when cl.Direction = 'Outbound' and cc.Name not in   
         ('NewCall', 'ContactCustomer', 'CustomerCallback', 'ContactVendor', 'VendorCallback', 'VendorSelection', 'ClosedLoop')  
         then 1 else 0 end) as OutboundContactsOTHER,  
      sum(case when ca.Name like '%Information%' then 1 else 0 end) as InfoContact,  
      sum(case when cr.Name = 'Verify Service' then 1 else 0 end) as VerifyServiceContact,  
      sum(case when cr.Name = 'ISP Selection' then 1 else 0 end) as ISPSelectionContact,              
      sum(case when cr.Name = 'Re-dispatch' then 1 else 0 end) as ReDispatchContact,  
      sum(case when ca.Name = 'OutOfWarranty' then 1 else 0 end) as OutOfWarrantyContact,  
      sum(case when ca.Name = 'TransferredCallToAgero' then 1 else 0 end) as TransferredCallToAgeroContact,  
      sum(case when ca.Name = 'TransferredCallToAAA' then 1 else 0 end) as TransferredCallToAAAContact  
    ...
    WHERE sr2.CreateDate > DATEDIFF(mm,-8, GETDATE())
    group by  
  ...
  Left Outer join dbo.VendorInvoice vi on vi.PurchaseOrderID = po.ID
  Left Outer join dbo.VendorInvoiceStatus vis on vis.ID = vi.VendorInvoiceStatusID
  
  WHERE Sr.CreateDate > DATEDIFF(mm,-8, GETDATE())
  ---- Exclude virtual deletes
  and (po.ID IS NULL or po.IsActive = 1)
  and (vi.ID IS NULL or vi.IsActive = 1)
