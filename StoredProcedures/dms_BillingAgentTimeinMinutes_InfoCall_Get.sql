IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[dms_BillingAgentTimeinMinutes_InfoCall_Get]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[dms_BillingAgentTimeinMinutes_InfoCall_Get]
GO

CREATE PROCEDURE [dbo].[dms_BillingAgentTimeinMinutes_InfoCall_Get]
...
exec dbo.dms_BillingAgentTimeinMinutes_InfoCall_Get @ProgramIDs, '01/01/2015', '02/01/2015', '1=1'
...
select	@SQLString = @SQLString + 'join vw_BillingServiceRequestsPurchaseOrders srpo on a.ServiceRequestID = srpo.ServiceRequestID and srpo.Dispatch = 0 '
