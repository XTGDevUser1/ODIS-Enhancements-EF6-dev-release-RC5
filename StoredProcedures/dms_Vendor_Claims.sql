...
*  10 - LessThanOrEquals
*******************/
  --EXEC dms_Vendor_Claims @VendorID = 9 
 IF EXISTS (SELECT * FROM dbo.sysobjects 
 WHERE id = object_id(N'[dbo].[dms_Vendor_Claims]')   		AND type in (N'P', N'PC')) 
 BEGIN
 DROP PROCEDURE [dbo].[dms_Vendor_Claims] 
 END 
 GO  
 SET ANSI_NULLS ON 
 GO 
 ...
 CREATE PROCEDURE [dbo].[dms_Vendor_Claims]( 
