IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[dms_Vendor_Dasboard_Profile_Completness]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[dms_Vendor_Dasboard_Profile_Completness]
GO
CREATE PROC [dbo].[dms_Vendor_Dasboard_Profile_Completness](@VendorID INT = NULL)
AS
BEGIN
SELECT 
Profile.Registered
