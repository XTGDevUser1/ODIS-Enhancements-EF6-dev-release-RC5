IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[dms_BillingPurchaseOrdersIncurred_ServiceAmountPlusMarkup_Get]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[dms_BillingPurchaseOrdersIncurred_ServiceAmountPlusMarkup_Get]
GO

CREATE PROCEDURE [dbo].[dms_BillingPurchaseOrdersIncurred_ServiceAmountPlusMarkup_Get]
...
Declare @MarkUpAdj nvarchar(50) = '1.20',
		@MarkUpCap nvarchar(50) = '75.00'
...
select	@SQLString = @SQLString + '     then (CASE WHEN v.VendorInvoicePaymentAmount*' + @MarkUpAdj + ' > ' + @MarkUpCap + ' THEN ' + @MarkUpCap + ' ELSE v.VendorInvoicePaymentAmount*' + @MarkUpAdj + ' END) ' 
select	@SQLString = @SQLString + '     else (CASE WHEN v.PurchaseOrderAmount*' + @MarkUpAdj + ' > ' + @MarkUpCap + ' THEN ' + @MarkUpCap + ' ELSE v.PurchaseOrderAmount*' + @MarkUpAdj + ' END) end ServiceAmount '
