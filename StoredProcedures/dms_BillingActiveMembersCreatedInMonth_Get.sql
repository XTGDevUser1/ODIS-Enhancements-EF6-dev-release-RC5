IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[dms_BillingActiveMembersCreatedInMonth_Get]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[dms_BillingActiveMembersCreatedInMonth_Get]
GO
CREATE PROCEDURE [dbo].[dms_BillingActiveMembersCreatedInMonth_Get]
...
exec dbo.dms_BillingActiveMembersCreatedInMonth_Get @ProgramIDs, '11/01/2015', '11/30/2015', '1=1'

-- FORD for Testing
declare @ProgramIDs as BillingDefinitionProgramsTableType
insert into @ProgramIDs(ProgramID) select ID from dbo.Program where clientid = 14
exec dbo.dms_BillingActiveMembersCreatedInMonth_Get @ProgramIDs, '11/01/2015', '11/30/2015', 'VINModelYear=''2010'''

-- FORD for Testing
declare @ProgramIDs as BillingDefinitionProgramsTableType
insert into @ProgramIDs(ProgramID) select ID from dbo.Program where Code = 'NMCA'
exec dbo.dms_BillingActiveMembersCreatedInMonth_Get @ProgramIDs, '11/01/2015', '11/30/2015', '1=1'
...
select	@SQLString = @SQLString + ' and convert(date, v.DMSMemberCreateDate) >= ''' + @RangeBeginDate_CHAR + ''' ' -- Create Date on/after period begin
select	@SQLString = @SQLString + ' and convert(date, v.DMSMemberCreateDate) <= ''' + @RangeEndDate_CHAR + ''' ' -- Create Date on/before period end
