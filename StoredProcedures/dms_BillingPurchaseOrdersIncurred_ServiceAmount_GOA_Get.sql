IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[dms_BillingPurchaseOrdersIncurred_ServiceAmount_GOA_Get]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[dms_BillingPurchaseOrdersIncurred_ServiceAmount_GOA_Get]
GO

CREATE PROCEDURE [dbo].[dms_BillingPurchaseOrdersIncurred_ServiceAmount_GOA_Get]
...
select	@SQLString = @SQLString + 'and v.IsGOA = 1 ' -- GOA PO
