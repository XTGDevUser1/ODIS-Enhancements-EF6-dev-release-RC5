...
/****** Object:  StoredProcedure [dbo].[dms_BillingDispatchEvents_CancelWithoutPO_Get]    Script Date: 02/08/2016 14:28:16 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[dms_BillingDispatchEvents_CancelWithoutPO_Get]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[dms_BillingDispatchEvents_CancelWithoutPO_Get]
GO
...
CREATE PROCEDURE [dbo].[dms_BillingDispatchEvents_CancelWithoutPO_Get]
...
select	@SQLString = @SQLString + 'and v.IsDispatched = 0 ' -- Dispatch intended
