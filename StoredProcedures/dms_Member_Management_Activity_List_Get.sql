IF EXISTS (SELECT * FROM dbo.sysobjects 
 WHERE id = object_id(N'[dbo].[dms_Member_Management_Activity_List_Get]')   		AND type in (N'P', N'PC')) 
 BEGIN
 DROP PROCEDURE [dbo].[dms_Member_Management_Activity_List_Get] 
 END 
 GO  
 SET ANSI_NULLS ON 
 GO 
 ...
 --EXEC   [dms_Member_Management_Activity_List_Get] @memberID=1, @pageSize=50
 CREATE PROCEDURE [dbo].[dms_Member_Management_Activity_List_Get](    
  @memberID INT = NULL -- TODO - Let's use this in the where clause.     
...
SET @commentCount = 0    
DECLARE @Case AS INT      
--SET @Case = (Select CaseID From Vendor Where ID = @vendorLocationID)      
...
SELECT @MemberEntityID =ID from dbo.Entity(NOLOCK) WHERE Name = 'Member'    
...
--INSERT INTO @PurchaseOrderResult Select ID From PurchaseOrder(NOLOCK) Where VendorID = @vendorLocationID    
...
WHERE     
 ...
 --OR 
 (CLL.EntityID = @MemberEntityID AND CLL.RecordID = @memberID)    
...
AND ELL.EntityID = @MemberEntityID AND ELL.RecordID = @memberID
--(    
...
--OR (ELL.EntityID = @MemberEntityID AND ELL.RecordID = @memberID)    
...
-- CONTACT LOGS    
 ...
 WHERE CLL.EntityID = @MemberEntityID AND CLL.RecordID = @memberID    
 --(CLL.EntityID = @InboundCallEntityID AND CLL.RecordID IN (Select ID From @InboundCallResult))    
 --OR (CLL.EntityID = @EmergencyAssistanceEntityID AND CLL.RecordID IN (Select ID From @EmergencyAssistanceResult))    
 --OR (CLL.EntityID = @CaseEntityID AND CLL.RecordID = @Case)    
 --OR (CLL.EntityID = @MemberEntityID AND CLL.RecordID = @memberID)    
 --OR (CLL.EntityID = @PurchaseOrderEntityID AND CLL.RecordID IN (Select ID From @PurchaseOrderResult))    
...
UNION ALL    
 ...
 WHERE C.EntityID = @MemberEntityID AND C.RecordID = @memberID    
 ...
 --OR (C.EntityID = @MemberEntityID AND C.RecordID = @memberID)    
