...
*******************/
 ...
 WHERE id = object_id(N'[dbo].[dms_Claim_Batch_List_Get]')   		AND type in (N'P', N'PC')) 
 BEGIN
 DROP PROCEDURE [dbo].[dms_Claim_Batch_List_Get] 
 END 
 GO  
 SET ANSI_NULLS ON 
 GO 
 ...
 GO 
 -- EXEC dms_Claim_Batch_List_Get
 CREATE PROCEDURE [dbo].[dms_Claim_Batch_List_Get]( 
...
SELECT      B.ID
            ...
            , SUM(ISNULL(C.PaymentAmount,0)) AS TotalAmount
            , B.MasterETLLoadID
...
WHERE B.BatchTypeID = (SELECT ID FROM BatchType WHERE Name = 'ClaimExport')
