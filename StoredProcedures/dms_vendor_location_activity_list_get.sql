 ...
 WHERE id = object_id(N'[dbo].[dms_vendor_location_activity_list_get]')   		AND type in (N'P', N'PC')) 
 BEGIN
 DROP PROCEDURE [dbo].[dms_vendor_location_activity_list_get] 
 END 
 GO  
 SET ANSI_NULLS ON 
 GO 
 ...
 GO 
 --EXEC dms_vendor_location_activity_list_get @vendorLocationID = 319,@whereClauseXML = '<ROW><Filter TypeOperator="11" TypeValue="Contact Log"></Filter></ROW>' 
  
 CREATE PROCEDURE [dbo].[dms_vendor_location_activity_list_get](    
  @vendorLocationID INT = NULL -- TODO - Let's use this in the where clause.     
...
SELECT @VendorLocationEntityID =ID from dbo.Entity(NOLOCK) WHERE Name = 'VendorLocation'    
SELECT @ContactLogEntityID = ID FROM dbo.Entity(NOLOCK) WHERE Name = 'ContactLog'    
    
 
INSERT INTO @PurchaseOrderResult Select ID From PurchaseOrder(NOLOCK) Where VendorLocationID = @vendorLocationID    
...
WHERE     
 (  
 (CLL.EntityID = @VendorLocationEntityID AND CLL.RecordID = @vendorLocationID)    
...
AND (    
(ELL.EntityID = @VendorLocationEntityID AND ELL.RecordID = @vendorLocationID)    
...
UNION ALL    
 ...
 WHERE     
 (C.EntityID = @VendorLocationEntityID AND C.RecordID = @vendorLocationID)    
