...
namespace Martex.DMS.Areas.Common.Controllers
{
    ...
    /// <summary>
    /// Emergency Assistance Controller
    /// </summary>
    public class EmergencyAssistanceController : VehicleBaseController
    {
        ...
        /// <summary>
        /// Populates the custom case phone location.
        /// </summary>
        /// <param name="detailsRecord">The details record.</param>
        /// <param name="locationResult">The location result.</param>
        ...
        /// <summary>
        /// Handles the mobile integration.
        /// </summary>
        /// <param name="callbackNumber">The callback number.</param>
        ...
        private CustomCasePhoneLocation HandleMobileIntegration(string callbackNumber, int? contactPhoneTypeID)
        {
            ...
            List<MobileCallData_Result> info = progFacade.GetMobileConfigurationResult(DMSCallContext.ProgramID, "service", "rule", callbackNumber, DMSCallContext.InboundCallID, null, null);
        ...
        /// </summary>
        /// <returns></returns>
        /// <exception cref="DMSException">InBoundCallID is not supplied.</exception>
        ...
        public ActionResult Index()
        {
            ...
            if (!model.EmergencyAssistance.Latitude.HasValue && !model.EmergencyAssistance.Longitude.HasValue && !string.IsNullOrEmpty(callBackNumberFromSession))
            {
                model.ResultType = PhoneLocationResultType.NO_RECORDS_FOUND;
                model.ResultTypeMessage = "There is no location information for this phone number";
            }
            ...
            if (model.EmergencyAssistance.VehicleTypeID.HasValue)
            {
                ...
                ViewData[StaticData.VehicleMake.ToString()] = GetVehicleMake(model.EmergencyAssistance.VehicleTypeID.Value);
                ViewData[StaticData.VehicleModel.ToString()] = GetVehicleModel(model.EmergencyAssistance.VehicleMake, model.EmergencyAssistance.VehicleTypeID.Value);
            }
            logger.InfoFormat("Finished retrieving details.");
            model.EmergencyAssistance.ContactPhoneTypeID = phoneTypeID;

            if (model != null && model.EmergencyAssistance != null && !string.IsNullOrEmpty(model.EmergencyAssistance.VehicleLicenseState) && model.EmergencyAssistance.VehicleLicenseStateCountryID.HasValue)
            {
                ViewData[StaticData.ProvinceAbbreviation.ToString()] = ReferenceDataRepository.GetStateProvinces(model.EmergencyAssistance.VehicleLicenseStateCountryID.GetValueOrDefault()).ToSelectListItem<StateProvince>(x => x.Abbreviation.Trim(), y => string.Format("{0} - {1}", y.Abbreviation.Trim(), y.Name.Trim()), true);
            }

            // Assign Call Back number from Member
            if (phoneTypeID > 0)
            {
                model.CallBackNumberType = phoneTypeID;
            }
            if (DMSCallContext.StartCallData != null && !string.IsNullOrEmpty(DMSCallContext.StartCallData.ContactPhoneNumber))
            {
                model.CallBackNumber = DMSCallContext.StartCallData.ContactPhoneNumber;
            }
            if (DMSCallContext.StartCallData != null && DMSCallContext.StartCallData.ContactPhoneTypeID.HasValue)
            {
                model.CallBackNumberType = DMSCallContext.StartCallData.ContactPhoneTypeID;
            }
        ...
        /// <param name="selectedVehicleTypeID">The selected vehicle type ID.</param>
        /// <returns></returns>
        /// <exception cref="DMSException">Model is not Null</exception>
        [HttpPost]
        [ValidateInput(false)]
        public ActionResult SaveEmergencyAssistance(EmergencyAssistanceModel model, int? selectedVehicleTypeID)
        {
            ...
            // Set the callback number and phoe type into session
            if (!string.IsNullOrEmpty(model.CallBackNumber) && model.CallBackNumberType.HasValue)
            {
                if (DMSCallContext.StartCallData == null)
                {
                    DMSCallContext.StartCallData = new CallInformation();
                }
                DMSCallContext.StartCallData.ContactPhoneTypeID = model.CallBackNumberType.Value;
                DMSCallContext.StartCallData.ContactPhoneNumber = model.CallBackNumber;
            }
        ...
        public JsonResult GetPSAPNumber(string latitude, string longitude)
        {
            ...
            if (result.IsError)
            {
                ...
                result.ErrorMessage = "The PSAP lookup service is temporarily unavailable. Please try again";
            }
            else
            {
                oResult.Status = OperationStatus.SUCCESS;
        ...
        [HttpPost]
        [ValidateInput(false)]
        public JsonResult _GetComboVehicleMake(string vehicleType)
        {
            logger.InfoFormat("EmergencyAssistanceController - _GetComboVehicleMake() - vehicleType : {0}", vehicleType);
            int vehicleTypeID = 0;
            int.TryParse(vehicleType, out vehicleTypeID);
            return Json(GetVehicleMake(vehicleTypeID), JsonRequestBehavior.AllowGet);
        }
        ...
        [HttpPost]
        [ValidateInput(false)]
        public JsonResult _GetComboVehicleModel(string make, string vehicleType)
        {

            int vehicleTypeID = 0;
            int.TryParse(vehicleType, out vehicleTypeID);
            return Json(GetVehicleModel(make, vehicleTypeID), JsonRequestBehavior.AllowGet);
        }
        ...
        [HttpPost]
        [ValidateInput(false)]
        public JsonResult _GetComboVehicleStringModel(string make)
        {
            ...
            IEnumerable<SelectListItem> list = ReferenceDataRepository.GetVehicleModel((int)VehicleTypes.Auto,make).Distinct(modelDistinct).OrderBy(a => a.Model).ToSelectListItem(x => x.Model.ToString(), y => y.Model.ToString());
