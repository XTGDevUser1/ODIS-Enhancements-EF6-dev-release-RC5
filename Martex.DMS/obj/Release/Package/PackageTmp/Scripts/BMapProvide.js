...
BingMaps.prototype = function () {
    ...
    var addPinClickHandler = function (e) {
        // KB: Ensure that the API is loaded successfully before performing any operation
    ...
    var createDirectionsManagerCallback = function (locations) {
            ...
            this.directionsManager.setRenderOptions(
        {
            ...
            //KB: The following options are making the route go crazy!!
    ...
    var getDirections = function (locations) {
        ...
        if (typeof (Microsoft) != "undefined") {
            if (locations.length > 1) {
                ...
                for (var index = 0; index < locations.length; index++) {
                    this.directionsManager.addWaypoint(new Microsoft.Maps.Directions.Waypoint({ address: locations[index].description, location: new Microsoft.Maps.Location(locations[index].latitude, locations[index].longitude) }));
                }                
        ...
        console.log(this.directionsManager._waypoints);
    }
    ...
    var init = function (mapDiv, directionsDiv) {
        ...
        if (typeof (Microsoft) == "undefined") {
            openAlertMessage("An error encountered while loading BING API, please try after sometime or check the behavior on API demo site and report error to Microsoft");
        }
        else {
            this.mapDiv = mapDiv;
            ...
            this.mapOptions = {
                ...
                // KB: Null check
                mapTypeId: typeof (Microsoft) != "undefined" ? Microsoft.Maps.MapTypeId.road : null,
                ...
                // KB: Null check
                //theme: (typeof (Microsoft) != "undefined") ? new Microsoft.Maps.Themes.BingTheme() : null
            };
            this.infoboxSeed = 0;
        }
    }
    ...
    var addPins = function (locations, hasInfobox, hasSetDestination, infoBoxOptions, pinColor) {
        ...
        if (typeof (Microsoft) != "undefined") {
            ...
            for (var i = 0; i < mapLocations.length; i++) {
                ...
                var pin;
                if (pinColor != null) {
                    pin = new Microsoft.Maps.Pushpin(mapLocations[i], { text: locations[i].pinText, color: pinColor });
                }
                else {
                    pin = new Microsoft.Maps.Pushpin(mapLocations[i], { text: locations[i].pinText });
                }

                if (hasInfobox) {
                    infoBoxOptions.visible = false;

                    //DEBUG: + " [ " + mapLocations[i].latitude + "," + mapLocations[i].longitude + " ]";
                    infoBoxOptions.description = locations[i].description;
                    infoBoxOptions.title = "Location";
                    var infobox = new Microsoft.Maps.Infobox(mapLocations[i], infoBoxOptions);
    ...
    var createSpatialDataQueryManagerCallback = function (location, searchText, callback) {
        this.spatialDataQueryManager = new Microsoft.Maps.SpatialDataService.QueryAPIManager();
        return getBusinesses.bind(this, location, searchText, callback)();
    }

    //KB: Get Businesses using Spatial Queries
    var getBusinesses = function (location, businessOptionToSearch, callback) {
        
        var sdsDataSourceUrl = 'https://spatial.virtualearth.net/REST/v1/data/f22876ec257b474b82fe2ffcb8393150/NavteqNA/NavteqPOIs';

        // Create a filterGroup combining filters.
        //Create local variable for namespace to reduce coding.
        var sds = Microsoft.Maps.SpatialDataService;

        //Create individual filters.
        var typeIdFilter = new sds.Filter('EntityTypeID', 'eq', businessOptionToSearch);
        ...
        Microsoft.Maps.SpatialDataService.QueryAPIManager.search(queryOptions, currentMap, function (data) {
            console.log(data);
            callback(data);
            //currentMap.entities.push(data);
        });
    }

    /* Using Spatial Query Manager */
    var searchBusiness = function (location, businessOptionToSearch, callback) {
        // KB: Ensure that the API is loaded successfully before performing any operation
        
        if (typeof (Microsoft) != "undefined") {
            var context = this;

            if (this.spatialDataQueryManager == null) {
                Microsoft.Maps.loadModule("Microsoft.Maps.SpatialDataService", {
                    callback: function () {
                        createSpatialDataQueryManagerCallback.bind(context, location, businessOptionToSearch, callback)();
                    }
                });
            }
            else {
                getBusinesses.bind(this, location, businessOptionToSearch, callback)();
            }
        }
    }
